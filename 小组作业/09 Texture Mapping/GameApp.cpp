...
void GameApp::UpdateScene(float dt)
{
	...
	if (m_KeyboardTracker.IsKeyPressed(Keyboard::D1) && m_CurrMode != ShowMode::WoodCrate)
	{
		...
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pWoodCrateback.GetAddressOf());
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pFlareback.GetAddressOf());
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pFlareAlphaback.GetAddressOf());
	}
	...
	/*else if (m_KeyboardTracker.IsKeyPressed(Keyboard::D3) && m_CurrMode != ShowMode::box)
	{
		// 播放木箱动画
		...
		m_pd3dImmediateContext->VSSetShader(m_pVertexShader3D.Get(), nullptr, 0);
		m_pd3dImmediateContext->PSSetShader(m_pPixelShader3D.Get(), nullptr, 0);
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, fire.GetAddressOf());
	}*/
	...
	else if (m_CurrMode == ShowMode::FireAnim)
	{
		...
		totDeltaTime += dt;
		if (totDeltaTime > 1.0f / 30)
		{
			totDeltaTime -= 1.0f / 30;
			m_CurrFrame = (m_CurrFrame + 1) % 89;
			m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pFireAnims[m_CurrFrame].GetAddressOf());
		}		
	}
	/*else if (m_CurrMode == ShowMode::box)
	{
		static float phi = 0.0f, theta = 0.0f;
		phi += 0.0001f, theta += 0.00015f;
		XMMATRIX W = XMMatrixRotationX(phi) * XMMatrixRotationY(theta);
...
void GameApp::DrawScene()
{
	...
	if (m_pd2dRenderTarget != nullptr)
	{
		...
		static const WCHAR* textStr = L"切换显示:\n1-多纹理正方体\n2-2D（希望师兄能看看源码有什么问题\n3-未完成（8是很会";
		m_pd2dRenderTarget->DrawTextW(textStr, (UINT)wcslen(textStr), m_pTextFormat.Get(),
			D2D1_RECT_F{ 0.0f, 0.0f, 600.0f, 200.0f }, m_pColorBrush.Get());
		static const WCHAR* textStr_1 = L"2020QG工作室图形组第4次小组培训\n区肇洲\n2020年4月23日";
		m_pd2dRenderTarget->DrawTextW(textStr_1, (UINT)wcslen(textStr_1), m_pTextFormat.Get(),
			D2D1_RECT_F{ 480.0f, 0.0f, 1200.0f, 400.0f }, m_pColorBrush.Get());
...
bool GameApp::InitResource()
{
	...
	// 初始化箱纹理
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\pr.dds", nullptr, m_pWoodCrate.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\dw.dds", nullptr, m_pFlare.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\fa.dds", nullptr, m_pFlareAlpha.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\ai.dds", nullptr, m_pWoodCrateback.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\au.dds", nullptr, m_pFlareback.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\dw.dds", nullptr, m_pFlareAlphaback.GetAddressOf()));
	...
	for (int i = 1; i <= 89; ++i)
	{
		wsprintf(strFile, L"Texture\\dududu\\%03d.bmp", i);
	...
	// 初始化用于PS的常量缓冲区的值
	// 这里只使用一盏点光来演示
	m_PSConstantBuffer.pointLight[0].position = XMFLOAT3(-5.0f, 3.0f, -10.0f);
	m_PSConstantBuffer.pointLight[0].ambient = XMFLOAT4(0.3f, 0.3f, 0.3f, 1.0f);
	m_PSConstantBuffer.pointLight[0].diffuse = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
