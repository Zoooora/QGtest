...
void GameApp::UpdateScene(float dt)
{
	...
	{
		text2 = L"已瞄准";
		m_BoundingSphere2.Center = XMFLOAT3((float)mouseState.x, (float)mouseState.y, 0.0f);
		m_Sphere2.SetWorldMatrix(XMMatrixTranslation((float)mouseState.x, (float)mouseState.y, 0.0f));
		ready = true;
	}

	if (ready == true && m_MouseTracker.leftButton == Mouse::ButtonStateTracker::PRESSED) {

	}

	//if (INTERSECTS(m_BoundingSphere2, m_Cylinder2)) {
...
void GameApp::DrawScene()
{
	...
	switch (m_SkyBoxMode)
	{
	case SkyBoxMode::Daylight: m_pDaylight->Draw(m_pd3dImmediateContext.Get(), m_SkyEffect, *m_pCamera); 
		break;
	...
	if (m_pd2dRenderTarget != nullptr)
	{
		...
		m_pd2dRenderTarget->DrawTextW(text_2.c_str(), (UINT32)text_2.length(), m_pTextFormat.Get(),
			D2D1_RECT_F{ 390.0f, 300.0f, 600.0f, 600.0f }, m_pColorBrush.Get());
		m_pd2dRenderTarget->DrawTextW(text2.c_str(), (UINT32)text2.length(), m_pTextFormat.Get(),
			D2D1_RECT_F{ 390.0f, 250.0f, 600.0f, 600.0f }, m_pColorBrush.Get());
...
bool GameApp::InitResource()
{
	...
	// 球体(预先设好包围球)
	m_Sphere2.SetModel(Model(m_pd3dDevice.Get(), Geometry::CreateSphere()));
	m_BoundingSphere2.Center = XMFLOAT3(5.0f, -2.0f, 0.0f);
	m_BoundingSphere2.Radius = 1.0f;

	m_Cylinder2.SetModel(Model(m_pd3dDevice.Get(), Geometry::CreateCylinder()));
	...
	m_Sphere2.SetWorldMatrix(XMMatrixTranslation(5.0f, -2.0f, 0.0f));
	
	// 地面
	model.SetMesh(m_pd3dDevice.Get(), Geometry::CreatePlane(XMFLOAT2(100.0f, 100.0f), XMFLOAT2(5.0f, 5.0f)));
	model.modelParts[0].material.ambient = XMFLOAT4(0.2f, 0.2f, 0.2f, 1.0f);
	...
	m_Cylinder2.SetWorldMatrix(XMMatrixTranslation(-5.0f, -1.99f, 0.0f));
